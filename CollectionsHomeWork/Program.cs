...
namespace CollectionsHomeWork
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Email: rhyta.com");
            foreach (var item in Actions.DebtbyEmail(debtors, "rhyta.com"))
            {
            ...
            Console.WriteLine("\n\n\nBeetwen ages 26 -> 36");
            foreach (var item in Actions.DebtByAge(debtors))
            {
                Console.WriteLine($"{item.FullName} { DateTime.Now.Year - item.BirthDay.Year} age {item.Email} {item.Address} {item.Phone} {item.Debt}");
            }

            Console.WriteLine("\n\n\nDebt More 5000");
            foreach (var item in Actions.DebtMoreAmount(debtors,5000))
            {
                Console.WriteLine($"{item.FullName} {item.Debt} {item.Phone}");
            }


            Console.WriteLine("\n\n\nFull Name with 15 characters and more than 1 Seven");
            foreach (var item in Actions.returnByNameCountansMobile(debtors,15,'7'))
            {
            ...
            Console.WriteLine("\n\n\nBorn In Winter");
            foreach (var item in Actions.BornInWinter(debtors))
            {
                Console.WriteLine($"{item.FullName} {item.BirthDay}");
            }


            Console.WriteLine("\n\n\nMore Debt Than Average");
            foreach (var item in Actions.MoreDebtThanAverage(debtors))
            {
                foreach (var it in item)
                {
                    Console.WriteLine($"{it.FullName} {it.Debt}");
                }
                Console.WriteLine();
            }
            ...
            Console.WriteLine("\n\n\nPhone Without 8");
            foreach (var item in Actions.NotFindEight(debtors,'8'))
            {
                Console.WriteLine($"{item.FullName.Substring(item.FullName.LastIndexOf(' '))} " +
                    $"{DateTime.Now.Year - item.BirthDay.Year} {item.Debt}");
            }
            Console.WriteLine($" Common Debt: {Actions.CommobDebt(Actions.NotFindEight(debtors,'8'))}");



            Console.WriteLine("\n\n\n3 Same Letter in fullname");
            foreach (var item in Actions.ThreeSameLetter(debtors))
            {
                Console.WriteLine($"{item.FullName}");
            }

            Console.WriteLine($"\n\n\nThe Year Made the Most Debtors {Actions.YearMadeDebtors(debtors)}");

            Console.WriteLine("\n\n\nThe Most Debts");
            foreach (var item in Actions.MostDebt(debtors,5))
            {
                Console.WriteLine($"{item.FullName} {item.Debt}");
            }
            Console.WriteLine("\n\n\nCommon Debt");
            Console.WriteLine(Actions.CommobDebt(debtors));


            Console.WriteLine("\n\n\nDebtors Who was living when Second World War happend");
            foreach (var item in Actions.SecondWorldWar(debtors))
            {
                Console.WriteLine($"{item.FullName} {item.BirthDay}");
            }
            ...
            Console.WriteLine("\n\n\nunique phone");
            foreach (var item in Actions.uniquePhonNumber(debtors))
            {
                Console.WriteLine($"{item.FullName} {item.Phone}");
            }


            Console.WriteLine("\n\n\nDebtor can debt back untill bithday");
            foreach (var item in Actions.DebtBack(debtors))
            {
                Console.WriteLine($"{item.FullName} {item.BirthDay} {item.Debt}");
            }


            Console.WriteLine("\n\n\nSmile");
            foreach (var item in Actions.JoinLetters(debtors, "smile"))
            {
                Console.WriteLine($"{item.FullName}");
            }
            Console.ReadKey();
        ...
        private static List<Debtor> debtors = new List<Debtor> {
            new Debtor("Shimley T. Qualls", DateTime.Parse("December 30, 1932"), "123-456-7890", "ShirleyTQualls@teleworm.us", "3565 Eagles Nest Drive Woodland, CA 95695", 240),
            new Debtor("Danielle M. Srier", DateTime.Parse("October 18, 1953"), "321-473-4178", "DanielleWGrier@rhyta.com", "1973 Stoneybrook Road Maitland, FL 32751", 3599),
